...
\####Summary of Dataset: There are 50400 observations and 6 variables in
the `instacart` dataset. One observation represents one minute of
activity for each day of the week over five weeks. The variables
include:

-   `week` representing which week data was collected from (range: 1-5)
-   `day_id` representing the number of the day out of the five weeks of
    data collection (range: 1-35)
-   `day` representing the day of the week
-   `weekend_v_weekday` describing whether the `day` recorded is the
    weekend or the weekday
-   `minute` depicting which the minute of the day the observation is
    representing (range: 1-1440)
-   `activity_counts` which represents the activity count calculated by
    the accelerometer (range: 1-8982).
