...
occurred on Sundays, while activity peaks around 1000 minutes on
Saturdays. Additionally, most activity around minute 1250 occurred on
...
The data is in ‘long’ format, and variables to note include:

-   `id`: a character variable representing the weather station ID
-   `date`: date of observation in YYYY-MM-DD format
-   `prcp`: an integer variable representing precipitation (tenths
    of mm) (range: NA-NA)
-   `snow`: an integer variable representing snowfall (mm) (range:
    NA-NA)
-   `snwd`: an integer variable representing snow depth (mm) (range:
    NA-NA)
-   `tmax`: a character variable depicting maximum temperature in tenths
    of degrees C (range: NA-NA; mean: NA)
-   `tmin`: a character variable depicting minimum temperature in tenths
    of degrees C (range: NA-NA; mean: NA)
...
The most commonly observed values for snowfall are `0` and `NA`. These
are the most commonly observed values for snowfall because snow can
typically only be expected in winter months; therefore, there may not be
snowfall to report in a majority of the months of the year. Stations may
have decided to report 0mm of snowfall or NA for days where there was
nothing to report.
...
(starting in 1990) appear to ebb and flow every 5 years. For example,
average `tmax` in 1990 was around 3 deg C, while the average `tmax` in
1995 was around -3 deg C. This noticeable deviation from 0 deg C
continues in the following years as well. Additionally, both maximum
temperatures recorded in January and June depict a handful of outliers.
Most notably, in January 2005, there was a recorded maximum value of 60
deg C. Similarly, in June 1990, there are two recorded maximum values
around -16 or -17 deg C.
