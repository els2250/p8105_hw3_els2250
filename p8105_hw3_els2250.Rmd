...
Reviewing the above table, there are a few noticeable trends/irregularities. First, Mondays appear to have the greatest activity counts each week. Second, the activity counts during the weekend appear to be lower compared to the other days of the week. Furthermore, the activity count for Saturday for week 4 and 5 is 1440, which is equivalent to the participant having an activity count of 1 for every minute of the day. This suggests that the participant may not have been wearing the accelerometer.  
...
Looking at the above graph, there appears to be a few trends according to the day of the week. Most activity between minutes 500 and 750 occurred on Sundays, while activity peaks around 1000 minutes on Saturdays. Additionally, most activity around minute 1250 occurred on weekdays and on Saturday.
...
#### Summary of Dataset: 
The `ny_noaa` dataset consists of `r nrow(ny_noaa)` rows and `r ncol(ny_noaa)` columns/variables. The data is in 'long' format, and variables to note include:

* `id`: a character variable representing the weather station ID
* `date`: date of observation in YYYY-MM-DD format
* `prcp`: an integer variable representing precipitation (tenths of mm) (range: `r min(ny_noaa$prcp)`-`r max(ny_noaa$prcp)`)
* `snow`: an integer variable representing snowfall (mm) (range: `r min(ny_noaa$snow)`-`r max(ny_noaa$snow)`)
* `snwd`: an integer variable representing snow depth (mm) (range: `r min(ny_noaa$snwd)`-`r max(ny_noaa$snwd)`)
* `tmax`: a character variable depicting maximum temperature in tenths of degrees C (range: `r min(ny_noaa$tmax)`-`r max(ny_noaa$tmax)`; mean: `r mean(ny_noaa$tmax, na.rm = TRUE)`)
* `tmin`: a character variable depicting minimum temperature in tenths of degrees C (range: `r min(ny_noaa$tmin)`-`r max(ny_noaa$tmin)`; mean: `r mean(ny_noaa$tmin, na.rm = TRUE)`)
...
#### Q3.1 Data Cleaning: *What are the most commonly observed values for snowfall? Why?*
...
The most commonly observed values for snowfall are `0` and `NA`. These are the most commonly observed values for snowfall because snow can typically only be expected in winter months; therefore, there may not be snowfall to report in a majority of the months of the year. Stations may have decided to report 0mm of snowfall or NA for days where there was nothing to report. 
...
While June appears to have a consistent average `tmax` around 30 deg C each year between 1980 and 2010, the average `tmax` values in January (starting in 1990) appear to ebb and flow every 5 years. For example, average `tmax` in 1990 was around 3 deg C, while the average `tmax` in 1995 was around -3 deg C. This noticeable deviation from 0 deg C continues in the following years as well. Additionally, both maximum temperatures recorded in January and June depict a handful of outliers. Most notably, in January 2005, there was a recorded maximum value of 60 deg C. Similarly, in June 1990, there are two recorded maximum values around -16 or -17 deg C. 
