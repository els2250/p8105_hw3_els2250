...
* `week` representing which week data was collected from (range: `r min(accelero_df$week)`-`r max(accelero_df$week)`)
* `day_id` representing the number of the day out of the five weeks of data collection (range: `r min(accelero_df$day_id)`-`r max(accelero_df$day_id)`)
* `day` representing the day of the week
* `weekend_v_weekday` describing whether the `day` recorded is the weekend or the weekday
* `minute` depicting which the minute of the day the observation is representing (range: 1-1440)
* `activity_counts` which represents the activity count calculated by the accelerometer (range: `r min(accelero_df$activity_counts)`-`r max(accelero_df$activity_counts)`).
