...
Looking at the above graph, there appears to be a few trends according to the week day. Most activity between minutes 500 and 750 occurred on Sundays, while activity peaks around 1000 minutes on Saturdays. Additionally, most activity around minute 1250 occurred on weekdays and on Saturday.
...
The most commonly observed values for snowfall are `0` and `NA`. **NEED TO SAY WHY**
...
ny_noaa_df %>% 
  ...
  geom_point(aes(color = tmax), alpha = 0.5) +
  geom_smooth(aes(x = year, y = tmax), se = FALSE, colour = "red") + 
  facet_grid(. ~ month) +
  labs(
